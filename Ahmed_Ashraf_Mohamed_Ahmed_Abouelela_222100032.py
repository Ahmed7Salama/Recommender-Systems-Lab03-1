import numpy as np 
def cosine_similarity(user1,user2):
    mask = (user1!=0)&(user2!=0)
    if not np.any(mask):
        return 0
    u1 = user1[mask]
    u2= user2[mask]
    dot_product=0
    for i in range(len(u1)):
        dot_product+=u1[i]*u2[i]
    sum_squares1 = 0
    for i in range(len(u1)):
        sum_squares1 += u1[i] ** 2
    magnitude1 = np.sqrt(sum_squares1)
    sum_squares2 = 0
    for i in range(len(u1)):
        sum_squares2 += u2[i]**2
    magnitude2 = np.sqrt(sum_squares2)
    if magnitude1 == 0 or magnitude2 == 0:
        return 0
    return dot_product/(magnitude1*magnitude2)

# Examples

user_A = np.array([5,3,0,4,0,2])
user_B = np.array([3,0,2,3,3,0])
user_C = np.array([4,3,4,3,5,3])

similarity_AB = cosine_similarity(user_A,user_B)
similarity_AC = cosine_similarity(user_A,user_C)
print("Cosine Similarity for User A and B",similarity_AB)
print("Cosine Similarity for User A and C",similarity_AC)